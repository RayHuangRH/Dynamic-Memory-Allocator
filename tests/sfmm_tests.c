...
Test(sf_memsuite_student, malloc_large_check_freelist, .init = sf_mem_init, .fini = sf_mem_fini) {//Malloc large amount
	sf_errno = 0;
	int *x = sf_malloc(10000);

	cr_assert_not_null(x, "x is NULL!");

	*x = 10000;

	cr_assert(*x == 10000, "sf_malloc failed to give proper space for a double!");
...
Test(sf_memsuite_student, coalesce_next_block_free, .init = sf_mem_init, .fini = sf_mem_fini){//Only next block is free
	sf_malloc(300);
    void* ptr2 = sf_malloc(8);
    void* ptr3 = sf_malloc(3980);
    sf_malloc(30);
...
Test(sf_memsuite_student, coalesce_next_prev_block_free, .init = sf_mem_init, .fini = sf_mem_fini){//Blocks on both sides are free
	sf_malloc(10);
    void* ptr2 = sf_malloc(60);
    void* ptr3 = sf_malloc(5000);
    void* ptr4 = sf_malloc(230);
    sf_malloc(630);
    ...
    assert_free_block_count(0, 2);
	assert_free_block_count(2096, 1);
	assert_free_block_count(5360, 1);
	cr_assert(sf_errno == 0, "sf_errno is not zero!");

}

Test(sf_memsuite_student, free_long, .init = sf_mem_init, .fini = sf_mem_fini){//Free all blocks for large quantity of data
    void* ptr1 = sf_malloc(780);
    void* ptr2 = sf_malloc(280);
    void* ptr3 = sf_malloc(1500);
    void* ptr4 = sf_malloc(980);
    void* ptr5 = sf_malloc(660);
    void* ptr6 = sf_malloc(7000);
...
Test(sf_memsuite_student, double_realloc_larger_smaller_block_splinter, .init = sf_mem_init, .fini = sf_mem_fini) {//realloc to larger and then to a smaller block
	void *x = sf_malloc(sizeof(int));
	...
	sf_block *bp = (sf_block *)((char*)y - 2*sizeof(sf_header));
	cr_assert(bp->header & THIS_BLOCK_ALLOCATED, "Allocated bit is not set!");
	cr_assert((bp->header & BLOCK_SIZE_MASK) == 32, "Block size not what was expected!");
